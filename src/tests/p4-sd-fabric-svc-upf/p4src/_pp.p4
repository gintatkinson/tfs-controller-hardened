...
@pa_container_size("egress" , "hdr.outer_udp.sport" , 16) @pa_container_size("egress" , "hdr.outer_udp.dport" , 16) header udp_t {
    bit<16> sport;
    bit<16> dport;
    bit<16> len;
    bit<16> checksum;
...
control Hasher(inout ingress_headers_t hdr, inout fabric_ingress_metadata_t fabric_md) {
    ...
    apply {
        ...
        if (fabric_md.bridged.upf.needs_gtpu_encap) {
            hash(fabric_md.ecmp_hash, HashAlgorithm.crc32, base, { fabric_md.bridged.upf.tun_peer_id, fabric_md.bridged.upf.teid }, max);
...
control UpfIngress(inout ingress_headers_t hdr, inout fabric_ingress_metadata_t fabric_md, inout standard_metadata_t standard_md, inout bool do_upf_uplink_recirc, inout bit<1> drop_ctl) {
    counter(4096, CounterType.packets_and_bytes) terminations_counter;
    meter(20000, MeterType.bytes) session_meter;
    meter(20000, MeterType.bytes) app_meter;
    ...
    @hidden action _drop_common() {
        drop_ctl = 1;
        fabric_md.skip_forwarding = true;
        fabric_md.skip_next = true;
    }
    @hidden action _term_hit(upf_ctr_idx_t ctr_id) {
        fabric_md.bridged.upf.upf_ctr_id = ctr_id;
        term_hit = true;
    }
    @hidden action _set_field_encap(teid_t teid, bit<6> qfi) {
        fabric_md.bridged.upf.needs_gtpu_encap = true;
        fabric_md.bridged.upf.teid = teid;
        fabric_md.bridged.upf.qfi = qfi;
    }
    @hidden action _gtpu_decap() {
    ...
    @hidden action _iface_common(slice_id_t slice_id) {
    ...
    action app_fwd(upf_ctr_idx_t ctr_id, tc_t tc, app_meter_idx_t app_meter_idx) {
        _term_hit(ctr_id);
        fabric_md.upf_tc = tc;
        app_meter_idx_internal = app_meter_idx;
        fabric_md.tc_unknown = false;
    }
    action downlink_fwd_encap(upf_ctr_idx_t ctr_id, tc_t tc, teid_t teid, bit<6> qfi, app_meter_idx_t app_meter_idx) {
...
control UpfEgress(inout ingress_headers_t hdr, inout fabric_egress_metadata_t fabric_md) {
    counter(4096, CounterType.packets_and_bytes) terminations_counter;
    action load_tunnel_params(l4_port_t tunnel_src_port, ipv4_addr_t tunnel_src_addr, ipv4_addr_t tunnel_dst_addr) {
    ...
    @hidden action _encap_initialize() {
    ...
    action gtpu_only() {
        _encap_common();
        hdr.ipv4.total_len = 20 + 8 + 8 + hdr.inner_ipv4.total_len;
        hdr.udp.len = 8 + 8 + hdr.inner_ipv4.total_len;
    ...
    action gtpu_with_psc() {
        ...
        _encap_common();
        hdr.ipv4.total_len = 20 + 8 + 8 + 4 + 4 + hdr.inner_ipv4.total_len;
        hdr.udp.len = 8 + 8 + 4 + 4 + hdr.inner_ipv4.total_len;
        hdr.gtpu.msglen = 4 + 4 + hdr.inner_ipv4.total_len;
        hdr.gtpu.ex_flag = 1;
