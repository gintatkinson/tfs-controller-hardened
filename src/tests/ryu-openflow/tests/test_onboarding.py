...
def test_scenario_onboarding(
    ...
    # Requires manual validation as number of onboarded devices is higher than those in descriptor;
    # the controller exposes underlying devices that are imported on the topology.
    ...
    #LOGGER.debug('Links: {:s}'.format(grpc_message_to_json_string(response)))
    num_mgmt_links = num_openvswitches
    num_ovs_2_ovs_links = 2*num_openvswitches # pentagon with bidirectional links
    assert len(response.links) == descriptor_loader.num_links + num_mgmt_links + num_ovs_2_ovs_links
