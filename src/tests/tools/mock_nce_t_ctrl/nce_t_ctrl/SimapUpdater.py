...
class SimapUpdater:
    ...
    def create_simap_trans_otn(self, etht_service : Dict) -> None:
        #etht_svc_name = etht_service['etht-svc-name']
        
        #src_node_ep   = etht_service['source-endpoints']['source-endpoint'][0]
        #src_node_id   = src_node_ep['node-id']
        #src_tp_id     = src_node_ep['tp-id']

        #dst_node_ep   = etht_service['destination-endpoints']['destination-endpoint'][0]
        #dst_node_id   = dst_node_ep['node-id']
        #dst_tp_id     = dst_node_ep['tp-id']

        simap = self._simap_client.network('trans-otn')
        simap.update(supporting_network_ids=['admin'])

        node_a = simap.node('site1')
        node_a.update(supporting_node_ids=[('admin', 'O-PE1')])
        node_a.termination_point('200').update(supporting_termination_point_ids=[('admin', 'O-PE1', '200')])
        node_a.termination_point('500').update(supporting_termination_point_ids=[('admin', 'O-PE1', '500')])
        node_a.termination_point('501').update(supporting_termination_point_ids=[('admin', 'O-PE1', '501')])

        node_b = simap.node('site2')
        node_b.update(supporting_node_ids=[('admin', 'O-PE2')])
        node_b.termination_point('200').update(supporting_termination_point_ids=[('admin', 'O-PE2', '200')])
        node_b.termination_point('500').update(supporting_termination_point_ids=[('admin', 'O-PE2', '500')])
        node_b.termination_point('501').update(supporting_termination_point_ids=[('admin', 'O-PE2', '501')])

        link_ab = simap.link('Trans-L1ab')
        link_ab.update(
            'site1', '500', 'site2', '500',
            supporting_link_ids=[
                ('admin', 'L7ab'), ('admin', 'L11ab'),
            ]
        )

        link_ba = simap.link('Trans-L1ba')
        link_ba.update(
            'site2', '500', 'site1', '500',
            supporting_link_ids=[
                ('admin', 'L11ba'), ('admin', 'L7ba'),
            ]
        )


    def delete_simap_trans_otn(self, etht_svc_name : str) -> None:
        simap = self._simap_client.network('trans-otn')
        simap.update(supporting_network_ids=['admin'])
