...
class OsuTunnels(Resource):
    def __init__(self, simap_updater : SimapUpdater):
        super().__init__()
        self._simap_updater = simap_updater

    def get(self):
        osu_tunnels = [osu_tunnel for osu_tunnel in OSU_TUNNELS.values()]
        data = {'ietf-te:te': {'tunnels': {'tunnel': osu_tunnels}}}
    ...
    def post(self):
        ...
        #te_data = json_request['ietf-te:te']
        #if not isinstance(te_data, dict): abort(400)
        #if 'tunnels' not in te_data: abort(400)
        #te_tunnels = te_data['tunnels']
        #if 'tunnel' not in te_tunnels: abort(400)
        #osu_tunnels = te_tunnels['tunnel']
...
class OsuTunnel(Resource):
    ...
    def post(self, osu_tunnel_name : str):
        json_request = request.get_json()
        if not json_request: abort(400)
        if not isinstance(json_request, dict): abort(400)
        #if 'ietf-te:te' not in json_request: abort(400)
        ...
        if len(osu_tunnels) != 1: abort(400)
        osu_tunnel = osu_tunnels[0]
        assert osu_tunnel_name == osu_tunnel['name']
        OSU_TUNNELS[osu_tunnel_name] = osu_tunnel
        return make_response(jsonify({}), 201)
