...
class IpowdmServiceHandler(_ServiceHandler):
    ...
    def SetEndpoint(
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                endpoint_ipowdm = self.__settings_handler.get_endpoint_ipowdm(device_obj, endpoint_obj)
                for _, endpoint in endpoint_ipowdm:
                    LOGGER.debug("[SetEndpoint] Found endpoint: %s", endpoint)
                    if endpoint["endpoint_id"]["device_id"]["device_uuid"]["uuid"] != "TFS-PACKET": continue
                LOGGER.debug("[SetEndpoint] endpoint_ipowdm: %s", endpoint_ipowdm)

                endpoint_name = endpoint_obj.name
                LOGGER.debug("[SetEndpoint] Endpoint name: %s", endpoint_name)

                json_config_rules = setup_config_rules(endpoint_name, endpoint_ipowdm)
                ...
                if len(json_config_rules) > 0:
                    ...
                    LOGGER.info("[SetEndpoint] Configuration sent for device %s", device_uuid)
                else:
