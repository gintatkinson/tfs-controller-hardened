...
class TaskExecutor:
    ...
    def get_devices_from_connection(
        ...
        plain_devices = {
            device_type : {
                device_uuid : grpc_message_to_json_string(device_grpc)
                for device_uuid, device_grpc in device_dict.items()
            }
            for device_type, device_dict in devices.items()
        }
        LOGGER.debug('[get_devices_from_connection] devices = {:s}'.format(str(plain_devices)))
    ...
    def get_device_type_drivers_for_connection(
        ...
        plain_devices = {
            device_type : {
                device_uuid : {
                    'grpc_object' : grpc_message_to_json_string(device_grpc),
                    'device_drivers' : list(device_drivers)
                }
                for device_uuid, (device_grpc, device_drivers) in device_dict.items()
            }
            for device_type, device_dict in devices.items()
        }
        LOGGER.debug('[get_device_type_drivers_for_connection] devices = {:s}'.format(str(plain_devices)))
        return devices
